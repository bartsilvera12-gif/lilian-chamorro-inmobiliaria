...
import { Plus, Pencil, Trash2, Loader2, Check, Search, Trophy } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function AdminSuccessCasesPage() {
  ...
  const fetchData = async () => {
    ...
    if (error) {
      toast.error("Error al cargar casos de éxito: " + error.message);
      setItems([]);
      setLoading(false);
      return;
    }

    setItems((data ?? []) as SuccessCaseRow[]);
    setLoading(false);
  };
  ...
  const openEdit = (row: SuccessCaseRow) => {
    setForm({
  ...
  const handleSave = async () => {
    if (!form.image_url.trim() || !form.description_es.trim()) {
      toast.error("Imagen URL y descripción en español son obligatorias");
      return;
    }
    ...
    let error: any;
    if (editingId) {
      ({ error } = await osorio.from("success_cases").update(payload).eq("id", editingId));
    } else {
      ({ error } = await osorio.from("success_cases").insert(payload));
    }
    ...
    toast.success(editingId ? "Caso de éxito actualizado" : "Caso de éxito creado");
    closeForm();
    fetchData();
  };

  const handleDelete = async (id: string) => {
    const { error } = await osorio.from("success_cases").delete().eq("id", id);
    if (error) {
      toast.error("Error: " + error.message);
      return;
    }
    ...
    <AdminLayout>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-foreground">Casos de Éxito</h1>
          <button
            onClick={openCreate}
        ...
        {showForm && (
          <div className="bg-card border border-border rounded-xl p-5 space-y-4">
            <h2 className="text-sm font-semibold text-foreground">{editingId ? "Editar Caso de Éxito" : "Nuevo Caso de Éxito"}</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="lg:col-span-2">
                <label className={labelCls}>Descripción * (solo español)</label>
                <textarea
                ...
                <p className="text-xs text-muted-foreground mt-1.5">
                  En el sitio público se traduce automáticamente al cambiar el idioma (inglés/portugués).
                </p>
              </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 text-muted-foreground">
            ...
            <p className="text-sm">No hay casos de éxito cargados</p>
          </div>
        ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/50">
                    ...
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">Descripción ES</th>
                  ...
                  {filtered.map((row) => (
                    <tr key={row.id} className="hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-3">
                        <img src={row.image_url} alt="Caso de éxito" className="w-20 h-12 rounded-md object-cover border border-border" />
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                <AlertDialogTitle>¿Eliminar caso de éxito?</AlertDialogTitle>
